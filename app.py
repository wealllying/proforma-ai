...
story.append(Paragraph(f"Deal: {cost//1000000}-Unit Multifamily | Your City", styles['Normal']))  # ← customize deal name
...
], colWidths=[3.5*72, 2.5*72])  # Adjust widths for fit
...
# IRR Distribution Table
story.append(Paragraph("EQUITY IRR DISTRIBUTION (50,000 SCENARIOS)", styles['Heading2']))
irr_table = Table([
    ["Percentile", "IRR"],
    ["5th percentile (severe downside)", f"{p[0]:.1%}    ← still positive"],
    ["25th percentile", f"{p[1]:.1%}"],
    ["Median (base case)", f"{p[2]:.1%}    ← target"],
    ["75th percentile", f"{p[3]:.1%}"],
    ["95th percentile (strong upside)", f"{p[4]:.1%}"]
...
st.download_button(
    ...
    pdf_bytes,
    f"ProForma_AI_Report_{cost//1000000}M.pdf",
    "application/pdf"
)
